...
impl Sugarloaf<'_> {
    ...
    /// Render to the offscreen texture instead of the window surface.
    /// Call ctx.enable_offscreen() first, then this method each frame.
    /// After rendering, use ctx.read_offscreen_pixels() to get the pixel data.
    pub fn render_offscreen(&mut self) {
        ...
        let mut encoder = self.ctx.device.create_command_encoder(
            &wgpu::CommandEncoderDescriptor { label: Some("sugarloaf_offscreen_render") },
        );
...

        {
            ...
            let mut rpass =
                encoder.begin_render_pass(&wgpu::RenderPassDescriptor {
                    timestamp_writes: None,
                    occlusion_query_set: None,
                    label: Some("sugarloaf_offscreen_pass"),
                    color_attachments: &[Some(wgpu::RenderPassColorAttachment {
                        view: &view,
                        resolve_target: None,
                        depth_slice: None,
