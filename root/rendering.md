...
**2. `history.pushState()` is the only navigation mechanism.** Bevy never calls `webview.load_url()` for routing — that is a full page reload that destroys Leptos state and fragments history. instead, Bevy sends IPC to Leptos, Leptos writes the URL, and Bevy reacts. one write path means one history. back and forward work everywhere, across aip switches and within-aip navigation alike.
...
**`history.pushState()` is the only navigation mechanism.** Bevy never calls `webview.load_url()` for routing. that is a full page reload — it destroys Leptos state and does not create a proper history entry. every navigation goes through Leptos.
...
Leptos: use_navigate() or <A href=...>
  ...
  → Bevy reacts: activates right surface if first segment changed
  → Leptos reacts: ContentArea renders right aip + sub-page
```
...
user presses Cmd+2
  ...
  → Bevy sends IPC to WebView: { navigate: "/brain" }
  → Leptos receives IPC → calls use_navigate()("/brain")
  → history.pushState("/brain")
  → wry fires navigation event
  → Bevy reacts: activates mir
  → Leptos reacts: ContentArea renders TransparentPane
```

Bevy delegates navigation to Leptos via IPC. the IPC round-trip is ~1ms — invisible to the user.
...
back/forward work across the full history — hotkey surface switches, link taps, sub-page navigation — because every navigation went through pushState.
...
| hotkey dispatch | Bevy | IPC → Leptos navigates |
