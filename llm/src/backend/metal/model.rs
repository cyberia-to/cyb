...
enum WeightFormat { Q4, Q4K, Q5K, Q6K, Q3K, Q2K, F16 }

impl WeightFormat {
    fn is_kquant(self) -> bool {
        matches!(self, WeightFormat::Q4K | WeightFormat::Q5K | WeightFormat::Q6K
                     | WeightFormat::Q3K | WeightFormat::Q2K)
    }

    fn kquant_pipeline<'a>(self, p: &'a super::MetalPipelines) -> &'a aruminium::Pipeline {
        match self {
            WeightFormat::Q6K => &p.matvec_q6k,
            WeightFormat::Q5K => &p.matvec_q5k,
            WeightFormat::Q3K => &p.matvec_q3k,
            WeightFormat::Q2K => &p.matvec_q2k,
            _ => &p.matvec_q4k,
        }
    }
}
...
impl MetalModel {
    ...
    pub fn from_loaded(lm: crate::cyb_format::LoadedModel) -> Result<Self, String> {
        ...
        let lm_head_format = match lm_w.dtype {
            ...
            crate::ir::DType::Q5_K => WeightFormat::Q5K,
        ...
        // Upload quantized weights — returns (buffer, format)
        let q4_upload = |name: &str, n: usize, k: usize| -> Result<(Buffer, WeightFormat), String> {
            let w = weights.get(name).ok_or_else(|| format!("Missing: {name}"))?;
            match w.dtype {
                crate::ir::DType::Q4 => {
                    let transposed = transpose_q4_blocks(&w.data, n, k);
                    Ok((pipelines.upload_bytes(&transposed).map_err(|e| format!("{e}"))?, WeightFormat::Q4))
                }
                crate::ir::DType::Q4_K => {
                ...
                crate::ir::DType::Q5_K => {
                    let transposed = transpose_blocks(&w.data, n, k, 256, 176);
                    Ok((pipelines.upload_bytes(&transposed).map_err(|e| format!("{e}"))?, WeightFormat::Q5K))
                }
                crate::ir::DType::Q6_K => {
                ...
                crate::ir::DType::Q3_K => {
                    let transposed = transpose_blocks(&w.data, n, k, 256, 110);
                    Ok((pipelines.upload_bytes(&transposed).map_err(|e| format!("{e}"))?, WeightFormat::Q3K))
                }
                crate::ir::DType::Q2_K => {
        ...
        unsafe { aruminium::autorelease_pool(|| {
            p.dispatcher.batch_raw(|batch| {
                ...
                for i in 0..c.num_layers {
                    ...
                    if c.is_ternary {
                    ...
                    } else if layer.weight_format.is_kquant() || layer.v_proj_format.is_kquant() {
                        ...
                        let dispatch_kquant = |batch: &aruminium::Batch, input: &Buffer, w: &Buffer, out: &Buffer, n: u32, k: u32, fmt: WeightFormat| {
                            let pipe = fmt.kquant_pipeline(p);
                            let params = [n, k];
                            batch.bind(pipe);
                            batch.bind_buffer(input, 0, 0);
                            batch.bind_buffer(w, 0, 1);
                    ...
                    // ── O projection ──
                    {
                        ...
                        if c.is_ternary {
                            ...
                            batch.push(bytemuck::cast_slice(&params), 3);
                            batch.launch_groups((div_ceil(n as usize, 4), 1, 1), (256, 1, 1));
                        } else if layer.o_proj_format.is_kquant() {
                            let kquant_pipe = layer.o_proj_format.kquant_pipeline(p);
                    ...
                    if c.is_ternary {
                        ...
                        batch.push(bytemuck::cast_slice(&f16_params), 3);
                        batch.launch_groups((div_ceil(inter as usize, 4), 1, 1), (256, 1, 1));
                    } else if layer.ffn_gate_up_format.is_kquant() {
                        // K-quant gate + up (separate dispatches, per-projection format)
                        let kquant_pipe = layer.ffn_gate_up_format.kquant_pipeline(p);
                        let inter = c.intermediate_size as u32;
                        let hid = c.hidden_size as u32;
                    ...
                    } else if layer.ffn_down_format.is_kquant() {
                        let kquant_pipe = layer.ffn_down_format.kquant_pipeline(p);
                        let params = [c.hidden_size as u32, c.intermediate_size as u32];
                        batch.bind(kquant_pipe);
                        batch.bind_buffer(&self.scratch.gate, 0, 0);
                        batch.bind_buffer(&layer.down_proj, 0, 1);
                ...
                } else if self.lm_head_format.is_kquant() {
                    let kquant_pipe = self.lm_head_format.kquant_pipeline(p);
                    batch.bind(kquant_pipe);
                    batch.bind_buffer(&self.scratch.hidden2, 0, 0);
...
/// Quantize f32 weights to Q4_K format (144 bytes per 256 values).
/// Layout: d(f16=2) + dmin(f16=2) + scales(12) + qs(128) = 144 bytes
/// Input: [N, K] row-major f32. Output: [N, K/256] Q4_K blocks (row-major).
fn quantize_f32_to_q4k(weights: &[f32], n: usize, k: usize) -> Vec<u8> {
    ...
    for row in 0..n {
        for sb in 0..super_blocks {
            ...
            // Find global max abs for d, and max negative for dmin
            let block_vals = &weights[base..base + 256];

            // Process 8 sub-blocks of 32 values
            let mut scales = [0u8; 12];
            let mut qs = [0u8; 128];

            // Simple quantization: find per-sub-block scale and min
            ...
            for j in 0..8 {
                ...
                if j < 4 {
                    ...
                    scales[j + 4] = (scales[j + 4] & 0xF0) | (sc & 0xF);
                    scales[j - 4] = (scales[j - 4] & 0x3F) | ((sc >> 4) << 6);
                    scales[j + 4] = (scales[j + 4] & 0x0F) | ((m & 0xF) << 4);
                    scales[j] = (scales[j] & 0x3F) | ((m >> 4) << 6);
                }
            }
            out[dst + 4..dst + 16].copy_from_slice(&scales);

            // Quantize values to 4-bit nibbles
            for grp in 0..4 {
                let sc1 = sub_scales[grp * 2];
                let m1 = sub_mins[grp * 2];
                let sc2 = sub_scales[grp * 2 + 1];
                let m2 = sub_mins[grp * 2 + 1];
                let inv_sc1 = if sc1 > 0.0 { 1.0 / sc1 } else { 0.0 };
                let inv_sc2 = if sc2 > 0.0 { 1.0 / sc2 } else { 0.0 };

                for l in 0..32 {
                    let v1 = block_vals[grp * 64 + l];
                    let v2 = block_vals[grp * 64 + 32 + l];
                    let q1 = ((v1 + m1) * inv_sc1).round().min(15.0).max(0.0) as u8;
                    let q2 = ((v2 + m2) * inv_sc2).round().min(15.0).max(0.0) as u8;
