...
fn hf_to_gguf(name: &str) -> String {
    ...
    if name == "lm_head.weight" { return "output.weight".into(); }
...
pub fn quantize_f32_to_q4k(weights: &[f32], n: usize, k: usize) -> Vec<u8> {
    ...
    for row in 0..n {
        for sb in 0..super_blocks {
            ...
            for grp in 0..4 {
                let inv_sc1 = if sub_scales[grp*2] > 0.0 { 1.0/sub_scales[grp*2] } else { 0.0 };
                let inv_sc2 = if sub_scales[grp*2+1] > 0.0 { 1.0/sub_scales[grp*2+1] } else { 0.0 };
                for l in 0..32 {
                    let q1 = ((block_vals[grp*64+l]+sub_mins[grp*2])*inv_sc1).round().min(15.0).max(0.0) as u8;
                    let q2 = ((block_vals[grp*64+32+l]+sub_mins[grp*2+1])*inv_sc2).round().min(15.0).max(0.0) as u8;
