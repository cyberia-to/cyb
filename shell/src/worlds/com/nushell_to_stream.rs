//! PipelineData → tade chunk adapter.
//!
//! Converts nushell's structured output types into typed tade chunks
...
use tade::{Chunk, sigil, render, encode_nested, table_chunk};
                ...
                while let Some(pos) = pending_line.iter().position(|&b| b == b'\n') {
                    ...
                    if !trimmed.is_empty() {
                        let chunk = Chunk::new(
                            sigil::SIG,
                            render::TEXT,
                            tade::bytes::Bytes::from(trimmed.to_owned().into_bytes()),
                        );
...
mod tests {
    ...
    fn record_becomes_struct_chunk() {
        ...
        let inner = tade::decode_nested(&c.payload);
