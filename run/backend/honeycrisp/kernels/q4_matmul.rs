...
//! Split nibble layout (matches GGUF Q4_0 variant in canonical/q4.rs):
//!   byte i holds value[i] (low nibble) and value[i+16] (high nibble)
//! Dequant: x[i] = (nibble[i] - 8) * scale_f / 8
//!
//! One SIMD group (32 threads) per (batch, row). Each thread strides over
//! K blocks and accumulates a partial sum, then simd_sum reduces.
...
kernel void kmain(
    ...
    for (uint blk = lane; blk < dims.n_blocks; blk += 32u) {
        ...
        // i16 scale, 2-byte aligned
        device const short *scale_p = (device const short *)(block);
        int scale_i = int(*scale_p);
        // dequant scale = scale_f / 8 = i16 / 256 / 8
        float scale = float(scale_i) * (1.0f / 256.0f / 8.0f);

        threadgroup const float *xb = &x_shared[blk * 32u];
        // 16 packed nibble bytes. byte i has value[i] (lo nibble) and value[i+16] (hi nibble).
        float acc = 0.0f;
        for (uint l = 0; l < 16u; l++) {
            uint byte = uint(block[2u + l]);
            int lo = int(byte & 0x0Fu) - 8;
            int hi = int((byte >> 4) & 0x0Fu) - 8;
            acc = fma(float(lo), xb[l], acc);
            acc = fma(float(hi), xb[l + 16u], acc);
        }
