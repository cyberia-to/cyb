...
fn register_hotkeys(world: &mut World) {
    ...
    let hk_legacy    = HotKey::new(Some(mods), Code::Digit6);
    let hk_interface = HotKey::new(Some(mods), Code::Digit7);
    ...
    manager.register(hk_legacy).expect("register Cmd+6");
    manager.register(hk_interface).expect("register Cmd+7");
